...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
                       explored: Set[Block]): Stream[(Block, List[Move])] = neighbors.filter(b => !explored.contains(b._1))
   ...
   */
  lazy val pathsFromStart: Stream[(Block, List[Move])] = from(Stream((startBlock, List.empty)), Set())

  /**
   * Returns a stream of all possible pairs of the goal block along
   * with the history how it was reached.
   */
  lazy val pathsToGoal: Stream[(Block, List[Move])] = pathsFromStart.filter(st => done(st._1))
