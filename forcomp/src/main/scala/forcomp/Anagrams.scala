...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.toList.groupBy((elem: Char) => elem)
      .toList.map((pair: (Char, List[Char])) => (pair._1, pair._2.size))
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = sentence :: Nil /*{
    if (sentence.isEmpty) List(List())
      ...
      for (comb <- combinations(sentenceOccurrences(sentence))) {
        val words: List[Word] = dictionaryByOccurrences.getOrElse(comb, Nil)
        if (words != Nil) {
          result = words :: result
        }
      }
