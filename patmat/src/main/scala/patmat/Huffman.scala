...
object Huffman {
  ...
  def sort(list: List[(Char, Int)]) : List[(Char, Int)] = {
    if (list.isEmpty) list
      ...
      val f = minFreq(list.tail, list.head)
      f :: sort(list.filter(_ != f))
    }
  }

  def minFreq(freqs: List[(Char, Int)], cur: (Char, Int)): (Char, Int) = {
    if (freqs.isEmpty) cur
    else {
      val f = freqs.head
      if (f._2 < cur._2) minFreq(freqs.tail, f) else minFreq(freqs.tail, cur)
    }
  }
  ...
  def combine(trees: List[CodeTree]): List[CodeTree] = {
    ...
    val a: CodeTree = trees.head
    val b: CodeTree = trees.tail.head
    val f: Fork = makeCodeTree(a, b)
    insert(f, trees.tail.tail)
  }
  ...
  def until(condition: List[CodeTree] => Boolean,
            action: List[CodeTree] => List[CodeTree])(arg: List[CodeTree]): List[CodeTree] = {
    if (condition(arg)) arg
    else until(condition, action)(action(arg))
  }
  ...
  def decode(tree: CodeTree, bits: List[Bit]): List[Char] = {
    if (bits.isEmpty) Nil
      ...
      case Fork(left, right, cs, w) =>
        if (bits.head == 1) decode(right, bits) ::: decode(tree, bits.tail)
        else decode(left, bits) ::: decode(tree, bits.tail)
    }
  }
  ...
  def encode(tree: CodeTree)(text: List[Char]): List[Bit] = {
    ...
    def encodeChar(tree: CodeTree)(elem: Char): List[Bit] = {
      tree match {
        case Leaf(c, w) => if (c == elem) Nil else throw new Error("no leaf with char " + elem)
        case Fork(left, right, cs, w) =>
          if (find(left, elem)) 0 :: encodeChar(left)(elem)
          else 1 :: encodeChar(right)(elem)
      }
    }

    def find(tree: CodeTree, elem: Char): Boolean = {
      tree match {
        case Leaf(c, w) => c == elem
        case Fork(left, right, cs, w) => find(left, elem) || find(right, elem)
      }
    }
  ...
  def encodeOne(table: CodeTable)(text: List[Char]) : List[Bit] = {
    if (text.isEmpty) Nil
    else codeBits(table)(text.head) ::: encodeOne(table)(text.tail)
  }
